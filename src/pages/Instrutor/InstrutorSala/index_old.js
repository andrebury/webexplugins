...
import { init as initWebex, rooms, memberships, messages } from 'webex';
import './style.css';
// import {init, rooms, memberships, messages} from 'webex'

function InstrutorSala() {
  ...
  function iniciar() {
    ...
    webex.once('ready', () => {
      ...
      webex.meetings.register()
        ...
        .finally(() => {
          setRegistrado(webex.meetings.registered
            ? true
            : false);
  ...
  .then(() => {
    ...
    webexObj.messages.on('created', (event) => console.log(event));
    webexObj.messages.on('deleted', (event) => console.log(event));
  })
